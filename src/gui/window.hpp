...
struct new_download_dialog : public Gtk::Dialog
{
    ...
    /**
     * The header bar used exclusively by the new download dialog, it provides an add and a cancel button for new
     * download requests.
     */
    struct cancel_or_add_header_bar : Gtk::HeaderBar
    {
        explicit cancel_or_add_header_bar(Gtk::Window* parent);
...
{
    explicit download_not_supported_dialog(Gtk::Window* parent);
};
...
{
    explicit url_parse_error_dialog(Gtk::Window* parent);
