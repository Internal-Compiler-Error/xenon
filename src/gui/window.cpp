...
xenon::gui::window::window() {
    ...
    // the list_store object actually stores data, the model only describes data
    // structure
...
void xenon::gui::window::notify(enum event event) {
    most_recent_event_ = event;
    download_scheduler_dispatcher_.emit();
}

void xenon::gui::window::update_list_store() {
...
void xenon::gui::window::on_new_download() {
...
xenon::gui::download_column_model &xenon::gui::download_column_model::get_instance() {
...
xenon::gui::new_download_dialog::new_download_dialog(window *window) : header_bar_{this}, window_{window} {
    set_titlebar(header_bar_);
...
xenon::gui::new_download_dialog::cancel_or_add_header_bar::cancel_or_add_header_bar(Window *parent) : parent_{parent} {
...
xenon::gui::download_not_supported_dialog::download_not_supported_dialog(Window *parent)
    : Gtk::MessageDialog{*parent,
                         "this download is not supported due to the lack of a Content-Length header",
                         false,
                         Gtk::MessageType::MESSAGE_ERROR,
                         Gtk::ButtonsType::BUTTONS_OK,
                         true} {}

xenon::gui::url_parse_error_dialog::url_parse_error_dialog(Window *parent)
    : Gtk::MessageDialog{
          *parent, "error while parsing url", false, Gtk::MessageType::MESSAGE_ERROR, Gtk::ButtonsType::BUTTONS_OK,
          true} {}
