...
void xenon::network::sub_download::start() {
    ...
    client.set_follow_location(true);
    auto content_retriever = [this](const char *data, uint64_t data_length) { return on_write(data, data_length); };
...
bool xenon::network::sub_download::on_write(const char *data, uint64_t length) {
    file_.write(data, static_cast<std::streamsize>(length));
    downloaded_size_ += length;
    progress_ = static_cast<int>(static_cast<double>(downloaded_size_) / static_cast<double>(total_size_) * 100);
...
xenon::network::download::download(int index, httplib::Response const &response, skyr::url request_url)
    ...
    sub_downloads_.reserve(static_cast<std::size_t>(sub_count_));
    ...
    }
    {
        ...
        auto &ref = sub_downloads_.emplace_back(*this, request_url_, sub_count_ - 1, file_name_, range, open_modes);
        progress_map_[&ref] = 0;
    }
}
xenon::network::download::~download() { complete_all_downloads(); }
...
double xenon::network::download::progress() { return total_progress_; }
void xenon::network::download::report_progress(xenon::network::sub_download *self, int progress) {
    ...
    total_progress_ =
        static_cast<int>(static_cast<double>(total_progress) / static_cast<double>((100 * sub_count_)) * 100);
}
bool xenon::network::download::is_done() const { return done_; }
...
void xenon::network::download_scheduler::stop() { stop_signal_received = true; }
